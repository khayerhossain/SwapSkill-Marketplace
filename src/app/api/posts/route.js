...
export async function DELETE(req) {
  try {
    ...
    if (!id || !userId)
      return NextResponse.json({ error: "Missing post ID or user ID" }, { status: 400 });
...
export async function PATCH(req) {
  try {
    ...
    // Add Comment
    if (body.comment) updatedFields.comments = [...(post.comments || []), body.comment];

    // Delete Comment
    if (body.deleteCommentId)
      updatedFields.comments = post.comments?.filter((c) => c.id !== body.deleteCommentId) || [];

    // Edit Comment
    if (body.editCommentId && body.text)
      updatedFields.comments = post.comments?.map((c) =>

...
export default function QuizPage() {
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      if (isQuizStarted) {
        e.preventDefault();
        e.returnValue =
          "Your quiz is in progress. Leaving will lose your progress!";
  ...
  const getErrorReason = () => {
    if (!profileId && !localStorage.getItem("currentProfileId"))
      return "Profile ID missing";
    if (!userId && !localStorage.getItem("currentUserId"))
      return "User ID missing";
    if (!category && !localStorage.getItem("currentCategory"))
